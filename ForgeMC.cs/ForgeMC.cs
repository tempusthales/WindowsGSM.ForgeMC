...
namespace WindowsGSM.Plugins
{
    public class ForgeMC
    {
        ...
        public Plugin Plugin = new Plugin
        {
            ...
            version = "1.0",
            url = "https://github.com/tempusthales/WindowsGSM.ForgeMC", // Github repository link (Best practice)
        ...
        public string StartPath => "LaunchServer.bat"; // Game server start path
        ...
        public async Task<Process> Install()
        {
            ...
            try
            {
                using (var webClient = new WebClient())
                {
                    ...
                    await webClient.DownloadFileTaskAsync($"https://files.minecraftforge.net/maven/net/minecraftforge/forge/{build}/forge-1.16.5-36.1.4-installer.jar", ServerPath.GetServersServerFiles(_serverData.ServerID, StartPath));
                }
            }
            catch (Exception e)
            {
        ...
        {
            // Get latest version and build at https://files.minecraftforge.net/maven/net/minecraftforge/forge/1.16.5-36.1.4/forge-1.16.5-36.1.4-installer.jar
            try
            {
                using (var webClient = new WebClient())
                {
                    var version = JObject.Parse(await webClient.DownloadStringTaskAsync("https://files.minecraftforge.net/maven/net/minecraftforge/forge/1.16.5-36.1.4/forge-1.16.5-36.1.4-installer.jar"))["versions"][0].ToString(); // "1.16.5-36.1.4"
                    var build = JObject.Parse(await webClient.DownloadStringTaskAsync($"https://files.minecraftforge.net/maven/net/minecraftforge/forge/{version}/forge-1.16.5-36.1.4-installer.jar"))["builds"]["latest"].ToString(); // "133"
                    return $"{version}/{build}";
                }
            }
            catch
